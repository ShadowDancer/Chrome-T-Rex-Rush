...
class QNetworkAgent(TensorflowAgent):
    ...
    def __init__(self, obs_space, ats_space):
        ...
        self.output = tf.layers.dense(x, a_size, activation_fn=tf.nn.softmax)
    ...
    def act(self, observation, action_space):
        ...
        action = np.random.choice(q_values[0],p=q_values)
    ...
    def next_episode(self, episode):
        ...
        memory_buffer[:,2] = self.discount_rewards(memory_buffer[:,2])
         # posumuj nagrody mniejszając znaczenia nagrody wraz z kolejnymi
         # akcjami
